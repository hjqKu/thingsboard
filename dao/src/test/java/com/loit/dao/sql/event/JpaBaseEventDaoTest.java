 ...
 */
package com.loit.dao.sql.event;
...
import com.loit.common.data.page.TimePageLink;
import com.loit.dao.AbstractJpaDaoTest;
import com.loit.dao.event.EventDao;
import com.loit.dao.service.AbstractServiceTest;
...
import static com.loit.common.data.DataConstants.ALARM;
import static com.loit.common.data.DataConstants.STATS;
...
public class JpaBaseEventDaoTest extends AbstractJpaDaoTest {
    ...
    public void findEvent() {
        ...
        String eventType = DataConstants.STATS;
        ...
        Assert.assertEquals("be41c7a2-31f5-11e7-9cfd-2786e6aa2046", event.getId().getId().toString());
    }
    ...
    public void findEventsByEntityIdAndEventTypeAndPageLink() {
        ...
        TimePageLink pageLink1 = new TimePageLink(30, null, null, true);
        List<Event> events1 = eventDao.findEvents(tenantId, new DeviceId(entityId1), DataConstants.ALARM, pageLink1);
        assertEquals(5, events1.size());

        TimePageLink pageLink2 = new TimePageLink(30, startTime, null, true);
        List<Event> events2 = eventDao.findEvents(tenantId, new DeviceId(entityId1), DataConstants.ALARM, pageLink2);
        assertEquals(5, events2.size());

        TimePageLink pageLink3 = new TimePageLink(30, startTime, endTime, true);
        List<Event> events3 = eventDao.findEvents(tenantId, new DeviceId(entityId1), DataConstants.ALARM, pageLink3);
        assertEquals(5, events3.size());

        TimePageLink pageLink4 = new TimePageLink(4, startTime, endTime, true);
        List<Event> events4 = eventDao.findEvents(tenantId, new DeviceId(entityId1), DataConstants.ALARM, pageLink4);
        assertEquals(4, events4.size());

        UUID idOffset = events3.get(2).getId().getId();
        TimePageLink pageLink5 = new TimePageLink(10, startTime, endTime, true, idOffset);
        List<Event> events5 = eventDao.findEvents(tenantId, new DeviceId(entityId1), DataConstants.ALARM, pageLink5);
    ...
    private long createEventsTwoEntitiesTwoTypes(UUID tenantId, UUID entityId1, UUID entityId2, long startTime, int count) {
        for (int i = 0; i < count / 2; i++) {
            String type = i % 2 == 0 ? DataConstants.STATS : DataConstants.ALARM;
