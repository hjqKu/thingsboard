...
public abstract class BaseAuthControllerTest extends AbstractControllerTest {
    ...
    public void testGetUser() throws Exception {
        ...
        .andExpect(MockMvcResultMatchers.jsonPath("$.email",is(SYS_ADMIN_EMAIL)));
        
        loginTenantAdmin();
        doGet("/api/auth/user")
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.authority",is(Authority.TENANT_ADMIN.name())))
        .andExpect(MockMvcResultMatchers.jsonPath("$.email",is(TENANT_ADMIN_EMAIL)));
        
        loginCustomerUser();
        doGet("/api/auth/user")
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.authority",is(Authority.CUSTOMER_USER.name())))
        .andExpect(MockMvcResultMatchers.jsonPath("$.email",is(CUSTOMER_USER_EMAIL)));
    }
