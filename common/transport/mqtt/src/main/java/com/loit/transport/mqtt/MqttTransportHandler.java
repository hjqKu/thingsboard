...
import com.loit.transport.mqtt.session.DeviceSessionCtx;
...
public class MqttTransportHandler extends ChannelInboundHandlerAdapter implements GenericFutureListener<Future<? super Void>>, SessionMsgListener {
    ...
    private void processAuthTokenConnect(ChannelHandlerContext ctx, MqttConnectMessage msg) {
        ...
        if (StringUtils.isEmpty(userName)) {
            ...
            transportService.process(TransportProtos.ValidateDeviceTokenRequestMsg.newBuilder().setToken(userName).build(),
    ...
    private void processDisconnect(ChannelHandlerContext ctx) {
        ...
        if (deviceSessionCtx.isConnected()) {
            transportService.process(sessionInfo, AbstractTransportService.getSessionEventMsg(TransportProtos.SessionEvent.CLOSED), null);
    ...
    private void checkGatewaySession() {
        TransportProtos.DeviceInfoProto device = deviceSessionCtx.getDeviceInfo();
        try {
    ...
    private void onValidateDeviceResponse(TransportProtos.ValidateDeviceCredentialsResponseMsg msg, ChannelHandlerContext ctx) {
        if (!msg.hasDeviceInfo()) {
            ...
            sessionInfo = TransportProtos.SessionInfoProto.newBuilder()
                    ...
                    .build();
            transportService.process(sessionInfo, AbstractTransportService.getSessionEventMsg(TransportProtos.SessionEvent.OPEN), null);
